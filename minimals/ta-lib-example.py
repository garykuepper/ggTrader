...
addplots = [
    ...
    mpf.make_addplot(ce.iloc[:, 3], type='line', linestyle='--',  width=1.0, label='ATR_buy',
                     secondary_y=False),
    mpf.make_addplot(ce.iloc[:, 4], type='line', linestyle='--',  width=1.0, label='ATR_sell',
                     secondary_y=False),
...
mpf.plot(
    ...
    xlim=[end - pd.Timedelta(days=120), end],
)
