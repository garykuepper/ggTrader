...
### Fix: sleeve weight applied twice, capping live deployment at ~26% instead of ~75%

- **Found**: `risk.py`'s `sleeve_slot_caps()` applied sleeve weight a second
  time on top of `sleeve_position_notional()` (which already scales
  per-position dollar size by `weight * scale`). The slot cap was
  `floor(weight_i * max_positions)` — squaring weight's effect on max
  deployable exposure (`cap_i * notional_i ~ weight_i**2`). With the live
  2026-08-03 rebalance (sp500 0.2855 / midcap400 0.2808 / nasdaq100 0.4337,
  scale 0.7574), full deployment across all 30 slots topped out at
  `scale * position_pct * sum(weight_i**2)` ≈ **26%** of equity — matching the
  observed live state: 30/30 positions held, 61% ($63.3k of $103.8k) idle
  cash.
- **Fixed**: every positive-weight sleeve now gets the SAME slot budget
  (`max_positions`, still 30 by default = `floor(1/position_pct)`), not a
  weight-scaled fraction of it — a low-weight sleeve holds proportionally
  smaller positions, not fewer of them, so it needs the same slot count to
  fully deploy its own allocated capital. `RiskConfig.max_positions` is
  redefined as a **per-sleeve** slot budget (was: global); the global cap is
  now derived as `max_positions * n_positive_sleeves` (90 for 3 sleeves,
  was a flat 30) via `RiskGuard.max_new_positions(current_count, weights)`
  (signature changed — now takes `weights`) and `sleeve_slot_caps()`.
  `sleeve_position_notional()` (per-position dollar sizing) is unchanged;
  pinned by a new regression test to today's $741/$729/$1,126
  sp500/midcap400/nasdaq100 dollar amounts at the live state. Full
  deployment at that state is now `scale * position_pct * max_positions *
  sum(weight_i)` ≈ **75%** of equity — the remaining idle cash is the
  intentional vol-targeting effect (`scale` < 1), not a bug. Confined to
  `src/ggTrader/paper/`; `src/ggTrader/lab/` (WFO/backtest) does not use
  `max_positions`/`sleeve_slot_caps`/`max_new_positions` and is unaffected.
  10 new/updated tests in `tests/paper/test_risk.py` (872 total passing).
- **Measured** (not forced): ran the live sleeve construction
  (`EnsembleSignal` + `simulate_signals`, `return_pf=True`) over a recent
  ~1.1yr window to sanity-check the `floor(1/position_pct) = 30`
  slots-per-sleeve assumption against the backtest's actual concurrent
  position count. It does **not** hold cleanly: sp500 (503 symbols) ran a
  raw mean of 55.6 concurrent positions (median 54, p95 120, max 176), and
  the count needed to hold 90% of invested capital (not dust) still
  averaged 44.8 (median 45, p95 94); midcap400 similarly averaged 34.9
  (90%-of-capital) vs. 30 assumed; nasdaq100 (101 symbols) averaged only
  12.8, capped by universe size, not capital. Root cause: the backtest
  sizes each entry as `size_type="percent"` of the *current* (shrinking)
  sleeve cash — an asymptotic scheme that supports far more, progressively
  smaller concurrent positions than a fixed slot count implies (mean total
  deployment measured only ~52%/46% of sleeve capital even at those high
  counts) — whereas the live trader sizes each entry as a **fixed** dollar
  amount off total portfolio value, for which `floor(1/position_pct) = 30`
  is the mathematically exact number of full-size slices needed to sum to
  ~99% of allocated capital. The two sizing conventions aren't directly
  comparable on position *count*, so 30 is kept as the live-formula-correct,
  conservative default rather than forcing the empirical (materially higher)
  vbt number — flagged here for follow-up rather than silently resolved.
  Measurement script not committed (one-off, `/tmp` scratch).
- **Deploy**: the live trader runs via `docker compose run --rm
  ggtrader_live ...` (`scripts/paper_trade.sh`) against
  `ghcr.io/garykuepper/ggtrader:latest`, which `COPY . .`s the repo into the
  image at build time — a `git pull` alone does **not** update the running
  container; this fix requires `docker compose build` (or a fresh image
  push) before the next `paper_trade.sh` cron run picks it up.
- **First-run burst**: the live account is currently 30/30 positions at the
  old (buggy) cap; on the first run after this deploys, the global cap
  jumps 30 → 90, so up to **~60 new buys** could fire in one run to fill the
  newly-available slots (bounded by `check_concentration` per-symbol and by
  how many signals actually fire that day). No throttle added — the
  validated backtest has none — this is a known, quantified one-time burst.
