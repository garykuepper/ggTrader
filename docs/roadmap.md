...
>
> 4. **The strategy-side headline figures are stale too.** Any bare citation
>    of Sharpe **1.12** (SP500 core baseline) or **1.14** (3-sleeve blend)
>    below predates both the anchor-leak fix and the day-shift fix, and
>    should be read as superseded unless a local note next to it says
>    otherwise. The 2026-08-19 reproduction (pinned window, anchor fix only)
>    already found core **0.97**, blend **0.68**. A further 2026-08-22
>    re-run on the corrected (day-shift-fixed) tape, pinned to the same
>    2021-01-31→2026-04-30 window, measured core **0.99**, blend **0.69**,
>    SPY **0.78** (`docs/research/_rebaseline_corrected_tape_20260822.json`,
>    in the main checkout). See
...
### What Worked
* **5-Indicator Voting (BB+RSI+EMA+MACD+VolBB)**: Our best strategy config. Combining these 5 indicators beats the S&P 500 index on a risk-adjusted basis (Sharpe ratio of 1.12 vs 0.58). ⚠️ *Superseded — see the note at the top of this file; corrected re-run measures core 0.99 vs SPY 0.78.*
