...
* **Overnight Gap Reversion** (🧪 Wiring confirmed, July 6): New orthogonal-to-price-momentum candidate — fades extreme overnight gap-down Z-scores (open vs. prior close), filled at the same bar's close per the existing voter convention. Registered as `overnight_gap` and smoke-tested end-to-end through the real WFO/gate pipeline on a 30-symbol SP500 subset (17 folds, 27 param combos): completed without errors, only 2/17 folds cleared both gates, OOS Sharpe -0.03 vs. SPY 0.58 baseline. **This is a wiring check only, not a verdict** — a 30-symbol subset is far too small a sample to judge the strategy on; the real research question (full SP500 universe, full grid, GO/NO-GO vs. the 1.12 Sharpe baseline) is a separate follow-up research pass.
