...
| **Future Research** | 🧪 Research | With entry filtering, exit rules, weighted voting, and now Kelly sizing all tested and rejected, the only remaining lever is a genuinely orthogonal direction: the parked crypto-carry sleeve (gated on >$10k capital + funding-data backfill). |
...
* **~~Cross-Sectional Idiosyncratic Volatility~~** (❌ Rejected, July 7): Candidate B from the July-6 strategy review — a weight-based, long-only sleeve equal-weighting the eligible universe's lowest-idiosyncratic-variance quintile each month (residual variance vs. the universe's own equal-weighted return, the same market-factor convention `compute_vol_scalar` already uses). Required first generalizing the gated `--wfo` harness to support weight-based strategies at all (`wfo.py`'s fold-sweep now dispatches on `target_kind`, validated against the existing `xs_momentum` strategy before this one was tested). Full honest WFO (SP500, 17 folds, 6 combos): **OOS Sharpe 0.57 vs. SPY 0.58** (CAGR 10.2% vs 13.0%), aggregate WFE 0.51 (barely clears the 0.50 floor), winner selected in only 4/17 folds. Doesn't approach the deployed 1.12-Sharpe core, so rejected as a standalone strategy. Notably different shape than the other rejections though: MaxDD -17.2% vs. SPY's -22.1% — a genuinely shallower drawdown consistent with the defensive-premium premise, echoing MidCap's "not deploy-clean alone, possible diversification sleeve" pattern rather than a flat noise rejection. Not pursued further here; would need its own correlation-vs-core study to judge as a blend candidate.
