...
| **Future Research** | 🧪 Research | Exit-rule optimization (profit targets / time stops vs the current RSI exit — the largest untested lever), weighted voting, and Kelly sizing (smart trade sizing). |
...
**Current Thesis (June 2026):** Combining multiple trading indicators (Bollinger Bands, RSI, EMA, MACD, and Volume Bollinger Bands) works because their failures are independent. When one indicator is wrong, the others usually don't vote, preventing bad trades. We closed our performance gap against the S&P 500 index not by trying to find a "magic" new signal, but by **adjusting our trade sizes** to 3% so our money doesn't sit idle in cash. This is the core lesson: our edge comes from **how we size and exit trades, not from picking which stock to buy** — entry-side machine-learning filtering was tested exhaustively and rejected (June 28). Our main areas for improvement are therefore **exit-rule optimization** (when to sell — the largest untested lever), diversification (blending large-cap and mid-cap stocks), and weighted voting. The nearest goal remains going live with the current validated config; this research runs in parallel and only changes the deployed system if it survives strict out-of-sample tests.
...
* **Exit-Rule Optimization** (🧪 Next): Sweep take-profit and time-based ("max-hold") exits against the current RSI-cross exit, in honest walk-forward. Reversion P&L is dominated by *when* you sell, this surface has never been tested, and it carries no machine-learning overfit risk. Trailing/hard stops are excluded — already proven harmful (§2b). Runs in parallel to go-live; only changes the deployed config if it beats the current Sharpe (~1.12) through the gates.
