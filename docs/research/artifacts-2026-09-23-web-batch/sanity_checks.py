...
def load(sym):
    q = (
        "copy (select timestamp::date d, close from ohlcv where venue='yfinance'"
        f" and interval='1d' and symbol='{sym}' order by 1) to stdout csv"
    )
    out = subprocess.run(
        ["docker", "exec", "ggtrader_db", "psql", "-U", "ggtrader", "-d", "ggtrader", "-Atc", q],
        capture_output=True,
        text=True,
        check=True,
    ).stdout
    s = (
        pd.read_csv(io.StringIO(out), names=["d", "c"], parse_dates=["d"])
        .drop_duplicates("d")
        .set_index("d")["c"]
    )
    return s
...
for f in fomc:
    ...
    if loc < len(idx) and idx[loc] == f and loc >= 1:
        day_before.add(idx[loc - 1])
...
for name, (a, b) in {
    ...
    for s in ["SPY", "IEF", "TLT"]:
        print(f"  buy&hold {s:4s}            ", stats(r[s][w]))
    for s in ["IEF", "TLT"]:
        p = (from_end <= 3).astype(float)
        print(
            f"  month-end last3 {s}      ",
            stats((r[s] * p)[w], p[w]),
            f" exposure {p[w].mean() * 100:.0f}%",
        )
        p = pre_fomc.astype(float)
        print(
            f"  pre-FOMC day-1 {s}       ",
            stats((r[s] * p)[w], p[w]),
            f" exposure {p[w].mean() * 100:.1f}%  events {int(p[w].sum())}",
        )
    ...
    for lbl, (L, F) in {"ToM [-1,+3]": (1, 3), "ToM [-4,+4]": (4, 4)}.items():
        p = ((from_end <= L) | (pos_in_month < F)).astype(float)
        print(
            f"  {lbl} SPY else cash     ",
            stats((r["SPY"] * p)[w], p[w]),
            f" exposure {p[w].mean() * 100:.0f}%",
        ...
        print(
            f"      SPY mean bp/day in-window {r['SPY'][w][p[w] == 1].mean() * 1e4:5.1f}"
            f"  out {r['SPY'][w][p[w] == 0].mean() * 1e4:5.1f}"
        )
    # rebalancing tilt: last 4 days hold IEF if SPY beat IEF month-to-date
    # (through day -5), else SPY; SPY otherwise
    mtd = (1 + r).groupby(ym).cumprod() - 1
    sig = (mtd["SPY"] - mtd["IEF"]).where(from_end == 5).groupby(ym).transform("max")
    hold_ief = (from_end <= 4) & (sig > 0)
    ret = r["SPY"].where(~hold_ief, r["IEF"])
    p = hold_ief.astype(float)
    ...
    var = r["SPY"].rolling(21).var() * 252
    c = var.expanding(252).median()
    wv = (c / var).clip(upper=1).shift(1)
    print(
        "  vol-managed SPY (1/var)  ",
        stats((r["SPY"] * wv)[w], wv[w]),
        f" avg exposure {wv[w].mean() * 100:.0f}%",
    )
    wv2 = (np.sqrt(c) / np.sqrt(var)).clip(upper=1).shift(1)
    print(
        "  vol-managed SPY (1/vol)  ",
        stats((r["SPY"] * wv2)[w], wv2[w]),
        f" avg exposure {wv2[w].mean() * 100:.0f}%",
    )
