...
Restates paper_snapshots.positions['MNST'] for 2026-08-26 through 2026-09-08
to reflect the corrected (2x) market_value/unrealized_pl, now that persistent
split-state correction (see trader.py's `_compute_split_corrections`) exists
going forward and would otherwise leave this historical window carrying the
broker's phantom (pre-split) P&L.
...
def main() -> None:
    ...
    with engine.connect() as conn:
        ...
        for run_date, positions in rows:
            ...
            conn.execute(
                text(
                    "UPDATE paper_snapshots SET positions = :positions WHERE run_date = :run_date"
                ),
