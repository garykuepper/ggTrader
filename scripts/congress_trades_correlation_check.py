"""Ad hoc diversification check: does congress_trades's OOS return stream
correlate with the deployed 5-voter ensemble core's OOS stream? Same
question idio_vol (2026-07-07), max_effect (2026-07-17), pead
(2026-07-17), and insider_cluster_buy (2026-07-19) were put through --
congress_trades's standalone long-window WFO is the strongest result of
the whole session so far (OOS Sharpe 0.89 vs SPY 0.77, gate pass 34/40 =
85%, stability 13/40 = 33%, all the highest seen this session), so this
check plus the matched-window/blend test that follows it carry more
weight than usual. Uses run_wfo (not harness.walkforward) for both
strategies since it doesn't touch the research DB -- this is a throwaway
correlation measurement, not a run worth persisting.
"""
...
def main() -> None:
    ...
    print("Running congress_trades WFO...", flush=True)
    ct_result = run_wfo(
        CongressTradeMirrorStrategy.name,
        ...
        grid=build_grid(CongressTradeMirrorStrategy),
    ...
    if not isinstance(ct_result, WfoResult) or not isinstance(ensemble_result, WfoResult):
        print(f"congress_trades: {ct_result}\nensemble: {ensemble_result}")
        return

    ct_ret = ct_result.oos_equity.pct_change().dropna()
    ens_ret = ensemble_result.oos_equity.pct_change().dropna()
    aligned = pd.concat([ct_ret, ens_ret], axis=1).dropna()
    aligned.columns = ["congress_trades", "ensemble"]
    if len(aligned) >= 2 and aligned["ensemble"].std() > 0:
        corr = aligned["congress_trades"].corr(aligned["ensemble"])
        beta = float(np.polyfit(aligned["ensemble"], aligned["congress_trades"], 1)[0])
        print(
