...
def process_file(args):
    ...
    try:
        ...
        for interval in intervals:
            ...
            # Add trades count
            resampled['trades'] = df.resample(interval, label='left', closed='left').size()
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description="Process raw Kraken trades to OHLC Parquet.")
    parser.add_argument("--raw_dir", default="data/raw", help="Directory containing raw CSVs")
    parser.add_argument("--parquet_root", default="data/parquet", help="Root directory for Parquet dataset")
    parser.add_argument("--sample", action="store_true", help="Process only a few files for testing")
