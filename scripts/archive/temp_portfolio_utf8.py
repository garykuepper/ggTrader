...
def run_strategy_equal_share(close, entries, exits, config, share=0.1):
    ...
    print(
        f"DEBUG: shapes - close: {close.shape}, entries: {entries.shape}, group_by size: {close.shape[1]}"
    )
...
def main():
    ...
    try:
        strategies["Volatility Weighted"], weights["Volatility Weighted"] = (
            run_strategy_volatility_weighted(close, entries, exits, raw_config)
        )
    except Exception as e:
