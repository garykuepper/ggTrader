...
def test_bear_market_forgives_zero_trades(mock_metrics, mock_engine_cls, bear_market_ohlcv):
    ...
    assert final_train_metrics.loc["combo_A"] == 0.0, "Combo A traded 0 times in a bear market; Sharpe should be coerced to 0.0"
    assert final_train_metrics.loc["combo_C"] == 0.0, "Combo C traded 0 times in a bear market; Sharpe should be coerced to 0.0"
    
    # Assert Combo B (which traded but lost) kept its real score
    assert final_train_metrics.loc["combo_B"] == -1.0, "Combo B traded > min_trades, should retain its -1.0 score"
...
def test_bull_market_rejects_zero_trades(mock_metrics, mock_engine_cls, bull_market_ohlcv):
    ...
    assert pd.isna(final_train_metrics.loc["combo_A"]), "Combo A traded 0 times in a bull market; should be rejected (NaN)"
    
    # Assert Combo B retained its real success score
    assert final_train_metrics.loc["combo_B"] == 2.0, "Combo B traded > min_trades, should retain its 2.0 score"
