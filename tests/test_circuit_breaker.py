...
def mock_engine_deps(tmp_path):
    ...
    with patch("ggTrader.core.base_execution_engine.setup_live_logger"), patch(
        "ggTrader.core.base_execution_engine.TradeTracker"
    ), patch("ggTrader.data.live.cached_loader.CachedExchangeLoader"):
        yield {"PERSISTENCE_PATH": persist_path, "REOPT_FLAG_PATH": reopt_path}
...
def test_circuit_breaker_blocks_entry(mock_engine_deps, dummy_results_file):
    ...
    with patch("pathlib.Path.exists", return_value=False):
        ...
        signals = {"BTC-USD": {"entry": True, "exit": False, "current_price": 50000.0}}
