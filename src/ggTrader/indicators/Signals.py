...
class Signals:
    ...
    def entry_signals(
        ...
        # PSAR buy signal
        # Note: vbt.pandas_ta('psar') returns a DataFrame with columns like 'PSARl_0.02_0.2', 'PSARs_0.02_0.2', etc.
        # But here we assume we can access the wrapper or specific output.
        # standard pandas_ta psar returns both 'PSARl_...' and 'PSARs_...' (long and short)
        ...
        # vbt's pandas_ta accessor often returns a GenericAccessor.
        # user code used `psar.psarl_below(c_flat)`. We must assume this is a vbt convenience method
        # or specific to the wrapper. If not, we might need adjustments.
        # Assuming existing code worked, we keep the logic but clean style.
    ...
    def calculate_ohlcv_signals(
        ...
        for symbol in symbols:
            ...
            # Ensure we are copying data to avoid SettingWithCopy warnings if applicable
    ...
    def trailing_stop_and_exits(
        ...
        entries_flat = pd.DataFrame(
            entries_flat_v, index=entries_flat.index, columns=entries_flat.columns
        )
...
def _atr_trailing_stop_long_ohlc_touch_2d_numba(
    ...
    """
    Numba-optimized ATR trailing stop logic.
    Enforces monotonicity (stop never moves down).
    """
    n, m = high_vals.shape
