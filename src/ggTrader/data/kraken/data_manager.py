"""Live Kraken market data access via ccxt."""
...
from .constants import STABLE_BASES, SYMBOL_MAPPING, kraken_map
...
class KrakenData:
    ...
    @staticmethod
    def fetch_ohlcv_df(
        exchange, symbol, timeframe="4h", limit=100, since=None, quote="USD"
    ):
        """
        ...
        ohlcv = exchange.fetch_ohlcv(
            f"{symbol}/{quote}", timeframe=timeframe, since=since, limit=limit
        )
    ...
    @staticmethod
    def top_kraken_by_volume(
        limit=20, only_usd=True, exclude_stables: bool = True, verbose=False
    ):
        kraken = ccxt.kraken()
        kraken.load_markets()
        ...
        symbols = [
            s for s in markets.keys() if (s.endswith("/USD") if only_usd else True)
        ]
        ...
        try:
            ...
            if verbose:
                print(
                    f"Bulk fetch_tickers failed: {e}. Falling back to per-symbol fetch."
                )
            for s in symbols:
                try:
        ...
        for s, t in tickers.items():
            ...
            if qv is not None:
                ...
                v = (
                    t.get("v", [0, 0])[1]
                    if isinstance(t.get("v"), (list, tuple))
                    else 0.0
                )
                p = (
                    t.get("p", [0, 0])[1]
                    if isinstance(t.get("p"), (list, tuple))
                    else 0.0
                )
    ...
    def print_top_kraken_by_volume(
        self, limit=20, only_usd=True, exclude_stables: bool = True, verbose=False
    ):
        top_volume = self.top_kraken_by_volume(
            limit=limit, only_usd=only_usd, exclude_stables=exclude_stables
        )
...
if __name__ == "__main__":
    ...
    for p in pairs:
        print(p)
    print(f"\nTop by Volume")
    kData.print_top_kraken_by_volume(
        limit=20, only_usd=True, exclude_stables=True, verbose=True
    )
