"""Kraken Spot broker adapter — wraps ccxt.kraken behind the ``Broker`` Protocol.

The adapter is a thin translator: domain types (Order, Fill, Ticker, Balance)
in, ccxt calls out. It does not retry, cache, or rate-limit — those concerns
live in higher layers (already handled by ``data/live/cached_loader.py`` for
market data; live trading would add its own retry policy if needed).
"""
...
def _to_ccxt_symbol(instrument: Instrument) -> str:
    """Canonical "BTC-USD" → ccxt "BTC/USD"; prefer venue_specific_id if set."""
...
def _dec(value: Any) -> Decimal:
    """Coerce any numeric / string into Decimal via str (avoids float artifacts)."""
    if value is None:
        return Decimal("0")
...
class KrakenSpotBroker:
    ...
    def fetch_open_orders(self, instrument: Optional[Instrument] = None) -> list[Order]:
        ...
        results: list[Order] = []
        for raw in raws:
            inst = instrument if instrument is not None else self._infer_instrument(raw)
            if inst is not None:
                results.append(self._raw_to_order(raw, inst))
        return results
        ...
        for raw in raws:
            ...
            fills.append(
                Fill(
                    ...
                    fee_currency=(raw.get("fee") or {}).get("currency") or inst.quote_currency,
                )
            )
        return fills
    ...
    def fetch_balance(self) -> dict[str, Balance]:
        ...
        # ccxt returns top-level meta keys (info/free/used/total) alongside currency dicts.
        for ccy, entry in raw.items():
            ...
            # Tolerate ccxt rounding: snap total to free+used if within 1 ulp.
            if free + used != total:
                total = free + used
    ...
    def _infer_instrument(self, raw: dict[str, Any]) -> Optional[Instrument]:
        """Best-effort instrument inference from a ccxt response. None if unknown.

        Phase 1 leaves this stub. Callers that need cross-instrument queries
        (open orders / fills without an instrument filter) will pass an
        instrument explicitly. A full implementation requires an instrument
        registry keyed by ccxt symbol — added when a use case requires it.
        """
        return None
